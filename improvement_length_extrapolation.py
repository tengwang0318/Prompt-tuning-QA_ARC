...
def generate_prompt(tokenizer, question, candidate_answers, prompt_type,
                    demonstrations, demonstration_embeddings, embedder, reverse: bool, nums
                    ):
    ...
    if reverse:
        ...
        for idx in indices_sorted:
            ...
            if num_tokens_from_string(prompt + temp + template, tokenizer) <= args.max_len - 100:
                ...
                templates.append((prompt + template).strip())
                prompt = ""
                nums -= 1
                if nums < 0: break
        #     else:
...
def main():
    ...
    for i in tqdm(range(num_samples), ncols=0, total=num_samples):
        ...
        sources = generate_prompt(tokenizer, question, candidate_answers, args.prompt_type,
                                  demonstrations, demonstration_embeddings, embedder, args.reverse, args.nums)
        # with open("write_prompt.txt",'w') as f:
        ...
        with torch.no_grad():
            for source in sources:
                if i == 0: print(f"prompt #{i}: {source}")
                encoding = preprocess([source], [target], tokenizer)

                outputs = model(**encoding)
                log_likelihood += outputs.loss * -1 * lambda_
                lambda_ *= 0.5
